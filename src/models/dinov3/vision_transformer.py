...
from .layers import LayerScale, PatchEmbed, RopePositionEmbedding, SelfAttentionBlock
from ..mlp import Mlp

logger = logging.getLogger("dinov3")

# Temporary SwiGLU implementation - we'll use regular MLP for now
class SwiGLUFFN(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.SiLU, drop=0.0, bias=True, **kwargs):
        super().__init__()
...
class DinoVisionTransformer(nn.Module):
    ...
    def forward(self, *args, is_training: bool = False, **kwargs) -> Dict[str, Tensor] | Tensor:
...
def vit_small(patch_size=16, n_storage_tokens=0, **kwargs):
...
def vit_base(patch_size=16, n_storage_tokens=0, **kwargs):
    model = DinoVisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
...
def vit_large(patch_size=16, n_storage_tokens=0, **kwargs):
    model = DinoVisionTransformer(
        patch_size=patch_size,
        embed_dim=1024,
        depth=24,
...
def vit_giant(patch_size=16, n_storage_tokens=0, **kwargs):
    model = DinoVisionTransformer(
        patch_size=patch_size,
        embed_dim=1536,
        depth=40,
