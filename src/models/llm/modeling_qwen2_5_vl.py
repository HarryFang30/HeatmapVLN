...
class VGGTEmbeddingMerger(nn.Module):
    def __init__(
        self,
        config: VGGTEmbeddingMergerConfig,
    ) -> None:
        super().__init__()
        self.config = config
...
class Qwen2_5_VL_VGGTForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.merger_config = VGGTEmbeddingMergerConfig(
            input_dim= 2 * self.vggt.aggregator.embed_dim, # vggt concat tokens after frame and global attention
            output_dim=self.config.hidden_size
        )
        self.vggt_embedding_merger = VGGTEmbeddingMerger(
    ...
    def get_vgg_embeds(self, media_input: torch.Tensor, media_type: str):
        ...
        Shape of media_input: (B, F, C, H, W)
        ...
        # -- shape of vgg_embeds: (Batch, Frame, Token_num, language_model_dim)
        vgg_embeds = self.vggt_embedding_merger(output_list, patch_start_idx, media_input.shape, media_type)
        vgg_embeds = vgg_embeds.view(-1, vgg_embeds.shape[-1]) # reshape to (S, D)
        return vgg_embeds
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        images_input: Optional[torch.Tensor] = None, # added to support vgg features
        videos_input: Optional[torch.Tensor] = None, # added to support vgg features
        pixel_values: Optional[torch.Tensor] = None,
        pixel_values_videos: Optional[torch.FloatTensor] = None,
        image_grid_thw: Optional[torch.LongTensor] = None,
        video_grid_thw: Optional[torch.LongTensor] = None,
        ...
        if inputs_embeds is None:
            ...
            # -- get vggt features --
            if images_input is not None or videos_input is not None:
                media_input = images_input if images_input is not None else videos_input
                media_type = "images" if images_input is not None else "video"
                media_input = media_input.type(self.vggt.dtype)
                vggt_embeds = self.get_vgg_embeds(media_input, media_type)

            if pixel_values is not None:
                pixel_values = pixel_values.type(self.visual.dtype)
                image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw)
                # -- fuse vggt features with image features --
                if vggt_embeds is not None:
                    image_embeds = image_embeds + vggt_embeds # TODO: more fusion methods 
                n_image_tokens = (input_ids == self.config.image_token_id).sum().item()
                n_image_features = image_embeds.shape[0]
                if n_image_tokens != n_image_features:
                    raise ValueError(
            ...
            if pixel_values_videos is not None:
                pixel_values_videos = pixel_values_videos.type(self.visual.dtype)
                video_embeds = self.visual(pixel_values_videos, grid_thw=video_grid_thw)
                # -- fuse vggt features with video features --
                if vggt_embeds is not None:
                    video_embeds = video_embeds + vggt_embeds # TODO: more fusion methods
                n_video_tokens = (input_ids == self.config.video_token_id).sum().item()
                n_video_features = video_embeds.shape[0]
                if n_video_tokens != n_video_features:
                    raise ValueError(
        ...
        if labels is not None:
            ...
            shift_labels = labels[..., 1:].contiguous()
            # Flatten the tokens
            loss_fct = CrossEntropyLoss(ignore_index=-100) #  ce loss ignore -100 index by default
            shift_logits = shift_logits.view(-1, self.config.vocab_size)
            shift_labels = shift_labels.view(-1)
            # Enable model parallelism
            shift_labels = shift_labels.to(shift_logits.device)
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        visual_keys = ["pixel_values", "image_grid_thw", "pixel_values_videos", "video_grid_thw", "videos_input", "second_per_grid_ts"]
...
__all__ = ["Qwen2_5_VL_VGGTForConditionalGeneration", "Qwen2_5_VLForConditionalGeneration", "Qwen2_5_VLModel", "Qwen2_5_VLPreTrainedModel"]
