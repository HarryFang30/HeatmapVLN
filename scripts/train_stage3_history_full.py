#!/usr/bin/env python3
"""
Stage 3: History全模块 (128)
训练：history_head, history_validity, mlp_fusion, temporal, encoder
"""
...
def main():
    parser = argparse.ArgumentParser(description='Stage 3: History全模块')
    ...
    stage_cfg = cfg['training']['stages'][2]  # stage3_history_full
    stage_name = stage_cfg['name']

    logger.info(f"\n{'='*80}")
    logger.info(f"Stage 3: History全模块 (128) - 端到端训练")
    logger.info(f"{'='*80}\n")
    ...
    # ⭐ FIX: Use dynamic checkpoint name from config instead of hardcoded
    prev_stage_name = cfg['training']['stages'][1]['name']
    prev_ckpt = Path(cfg['log']['out_dir']) / f"{prev_stage_name}_best.pth"
    if prev_ckpt.exists():
        load_checkpoint(model, str(prev_ckpt), device)
    else:
        logger.warning(f"⚠️ Stage 2 checkpoint not found: {prev_ckpt}")
    ...
    if rank == 0 and cfg['log'].get('use_tensorboard', False):
        ...
        tb_writer = SummaryWriter(log_dir=str(tb_dir / f'stage3_{timestamp}'))
        logger.info(f"📊 TensorBoard enabled: {tb_dir / f'stage3_{timestamp}'}")

    best_val_loss = float('inf')
    stage_idx = 2  # ⭐ Stage 3 对应 idx=2
    ...
    if rank == 0:
        ...
        logger.info(f"\n✅ Stage 3 completed! Best val loss: {best_val_loss:.4f}")
        logger.info(f"下一步: python train_stage4_future_warmup.py --config {args.config}\n")
