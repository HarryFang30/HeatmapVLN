#!/usr/bin/bash -p
# Direct, foreground entrypoint for a web-scheduled one-node, four-GPU
# native InternNav trajectory-DAgger collection. The web platform allocates
# the node; this script only manages the four independent local workers.
...
# Request one node with 4 GPUs (recommended: 64 CPU, 128GB RAM, 60h) in the
# web form, then run this script directly in the foreground. Do not use nohup
# or a top-level '&'. Set DAGGER_4GPU_PREPARE_ONLY=1 only for a no-GPU preflight.
...
readonly NUM_SHARDS=4
readonly TOTAL_EPISODES=10819
readonly SEED=17
readonly ABSOLUTE_TOTAL_LIMIT_BYTES=300000000000
readonly WRAPPER_RESERVE_BYTES=5000000000

RUN_TAG="${DAGGER_4GPU_RUN_TAG:-full_train_4way_seed17}"
RUN_INSTANCE_ID="${DAGGER_4GPU_RUN_INSTANCE_ID:-}"
if [[ "${DAGGER_4GPU_GPU_DEVICES+x}" == x ]]; then
  GPU_DEVICES="$DAGGER_4GPU_GPU_DEVICES"
  GPU_DEVICE_SOURCE="DAGGER_4GPU_GPU_DEVICES"
elif [[ -n "${CUDA_VISIBLE_DEVICES:-}" ]]; then
  GPU_DEVICES="$CUDA_VISIBLE_DEVICES"
  GPU_DEVICE_SOURCE="inherited CUDA_VISIBLE_DEVICES"
else
  GPU_DEVICES="0,1,2,3"
  GPU_DEVICE_SOURCE="default logical devices 0..3"
fi
BASE_RPC_PORT="${DAGGER_4GPU_BASE_RPC_PORT:-53000}"
BASE_DISPLAY_NUM="${DAGGER_4GPU_BASE_DISPLAY_NUM:-400}"
STAGGER_SECONDS="${DAGGER_4GPU_STAGGER_SECONDS:-45}"
PER_SHARD_MAX_BYTES="${DAGGER_4GPU_MAX_BYTES_PER_SHARD:-30000000000}"
PER_SHARD_SOFT_BYTES="${DAGGER_4GPU_SOFT_BYTES_PER_SHARD:-25000000000}"
PREPARE_ONLY="${DAGGER_4GPU_PREPARE_ONLY:-0}"
LP_THREADS="${DAGGER_4GPU_LP_NUM_THREADS:-8}"
...
case "${PREPARE_ONLY,,}" in
  ...
  *) die "DAGGER_4GPU_PREPARE_ONLY must be an explicit boolean" ;;
esac
[[ "$GPU_DEVICES" =~ ^[0-9]+(,[0-9]+){3}$ ]] || \
  die "GPU mapping must contain exactly 4 comma-separated decimal device ids"
IFS=',' read -r -a GPU_LIST <<< "$GPU_DEVICES"
[[ "${#GPU_LIST[@]}" -eq "$NUM_SHARDS" ]] || die "exactly 4 comma-separated GPU ids are required"
declare -A GPU_SEEN=()
...
PY
  ...
  if [[ "$actual_token" != "$RUN_LOCK_TOKEN" ]]; then
    echo "[dagger-4gpu] refusing to remove run lock with mismatched owner: $RUN_LOCK_DIR" >&2
    return 0
  fi
  ...
  if ! rmdir -- "$RUN_LOCK_DIR"; then
    echo "[dagger-4gpu] run lock directory retained because it is not empty: $RUN_LOCK_DIR" >&2
    return 0
  fi
  RUN_LOCK_OWNED=0
  echo "[dagger-4gpu] released run lock: $RUN_LOCK_DIR"
}
...
PY
  ...
  echo "[dagger-4gpu] acquired atomic run lock: $RUN_LOCK_DIR"
fi
...
echo "[dagger-4gpu] building or verifying exact full-train shard plan"
...
echo "[dagger-4gpu] launcher_mode=web-direct run_instance_id=$RUN_INSTANCE_ID"
echo "[dagger-4gpu] gpu_devices=$GPU_DEVICES source=$GPU_DEVICE_SOURCE"
if is_true "$PREPARE_ONLY"; then
  echo "[dagger-4gpu] prepare-only complete; no GPU process was launched"
  exit 0
fi
...
cleanup_workers() {
  ...
  for pid in "${!ACTIVE_WORKERS[@]}"; do
    pgid="${WORKER_PGIDS[$pid]:-}"
    if [[ "$pgid" =~ ^[1-9][0-9]*$ ]] && process_group_alive "$pgid"; then
      echo "[dagger-4gpu] residual process group=$pgid; retaining run lock" >&2
      CLEANUP_RESIDUAL=1
      continue
    fi
...
top_level_cleanup() {
  ...
  if (( CLEANUP_RESIDUAL == 0 )); then
    ...
    echo "[dagger-4gpu] run lock retained because worker processes survived SIGKILL" >&2
  fi
  exit "$status"
}
trap top_level_cleanup EXIT
trap 'echo "[dagger-4gpu] received termination signal" >&2; exit 130' INT TERM HUP
...
reap_finished_workers() {
  ...
  for pid in "${finished[@]}"; do
    ...
    if process_group_alive "$pgid"; then
      echo "[dagger-4gpu] worker=$name leader exited but process group remains alive" >&2
      (( status == 0 )) && status=1
    else
      unset "ACTIVE_WORKERS[$pid]" "WORKER_NAMES[$pid]" "WORKER_PGIDS[$pid]"
    fi
    ...
    echo "[dagger-4gpu] worker=$name exited status=$status"
  done
}
...
while IFS='=' read -r -d '' inherited_name _; do
  case "$inherited_name" in
    BASH_FUNC_*|DAGGER_4GPU_*|DAGGER_8GPU_*|STAGE3_EVAL_*|HEATMAP_SYSTEM1_*|INTERNNAV_NATIVE_*|HEATMAPVLN_*|MACA_*|__GLX_*|__EGL_*|EGL_*|GLX_*|LIBGL_*|GALLIUM_*|MESA_*|HABITAT_*|MAGNUM_*|GBM_*)
      CHILD_ENV_UNSET_ARGS+=(-u "$inherited_name")
      ;;
  esac
done < <("$ENV_BIN" -0)

echo "[dagger-4gpu] launching 4 independent native InternNav workers"
echo "[dagger-4gpu] collection_base=$COLLECTION_BASE"
echo "[dagger-4gpu] control_base=$CONTROL_BASE"
echo "[dagger-4gpu] per_shard_hard=$PER_SHARD_MAX_BYTES per_shard_soft=$PER_SHARD_SOFT_BYTES"
for shard_index in $(seq 0 $((NUM_SHARDS - 1))); do
  ...
  echo "[dagger-4gpu] started $shard_name pid=$pid pgid=$pgid gpu=$gpu rpc=$rpc_port display=$display_num log=$log_path"
...
echo "[dagger-4gpu] all shards exited successfully; finalizing aggregate training roots"
...
echo "[dagger-4gpu] READY: $TRAINING_ROOTS_MANIFEST"
