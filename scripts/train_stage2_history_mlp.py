#!/usr/bin/env python3
"""
Stage 2: History + MLP融合 (64→128)

目标：训练MLP层进行特征融合和时序理解
训练：history_head, history_validity, mlp_fusion
冻结：encoder, temporal, future_head
...
def main():
    parser = argparse.ArgumentParser(description='Stage 2: History + MLP融合')
    ...
    stage_cfg = cfg['training']['stages'][1]  # stage2_history_fusion
    stage_name = stage_cfg['name']

    logger.info(f"\n{'='*80}")
    logger.info(f"Stage 2: History + MLP融合 (64→128)")
    logger.info(f"{'='*80}\n")
    ...
    prev_ckpt = Path(cfg['log']['out_dir']) / "stage1_history_warmup_best.pth"
    if prev_ckpt.exists():
        load_checkpoint(model, str(prev_ckpt), device)
    else:
        logger.warning("⚠️ Stage 1 checkpoint not found! Training from scratch.")
    ...
    if rank == 0 and cfg['log'].get('use_tensorboard', False):
        ...
        tb_writer = SummaryWriter(log_dir=str(tb_dir / f'stage2_{timestamp}'))
        logger.info(f"📊 TensorBoard enabled: {tb_dir / f'stage2_{timestamp}'}")

    best_val_loss = float('inf')
    stage_idx = 1  # ⭐ Stage 2 对应 idx=1
    ...
    if rank == 0:
        ...
        logger.info(f"\n✅ Stage 2 completed! Best val loss: {best_val_loss:.4f}")
        logger.info(f"下一步: python train_stage3_history_full.py --config {args.config}\n")
