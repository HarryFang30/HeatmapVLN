...
export STAGE3_EVAL_MODEL_GPU="${STAGE3_HYBRID_FIXED3_GPU:-7}"
export STAGE3_EVAL_SIM_GPU="$STAGE3_EVAL_MODEL_GPU"
export STAGE3_EVAL_RPC_PORT="${STAGE3_HYBRID_FIXED3_RPC_PORT:-50075}"
export STAGE3_EVAL_DISPLAY="${STAGE3_HYBRID_FIXED3_DISPLAY:-localhost:200.0}"
export STAGE3_EVAL_OUTPUT_PATH="${STAGE3_HYBRID_FIXED3_OUTPUT_PATH:-${FJL_ROOT}/model/eval_system2_hybrid_stop_policy_fixed3_20260717}"
...
if [[ -e "$STAGE3_EVAL_OUTPUT_PATH" ]]; then
  echo "Refusing to overwrite hybrid fixed3 output: $STAGE3_EVAL_OUTPUT_PATH" >&2
  exit 1
fi

echo "[$(date '+%F %T')] launching hybrid STOP fixed3 on GPU $STAGE3_EVAL_MODEL_GPU"
echo "[hybrid-fixed3] static_add=$STATIC_CHECKPOINT"
echo "[hybrid-fixed3] temporal_veto=$TEMPORAL_CHECKPOINT"
