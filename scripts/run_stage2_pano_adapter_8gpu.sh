...
STAGE2_ADAPTER_DATASET_MAX_CLIPS="${STAGE2_ADAPTER_DATASET_MAX_CLIPS:-0}"
...
STAGE2_ADAPTER_STARTUP_PREFLIGHT_BATCHES="${STAGE2_ADAPTER_STARTUP_PREFLIGHT_BATCHES:-}"
...
build_adapter_args() {
  ...
  if [[ -n "${STAGE2_ADAPTER_DATASET_MAX_CLIPS:-}" && "${STAGE2_ADAPTER_DATASET_MAX_CLIPS}" -gt 0 ]]; then
    args+=(--dataset-max-clips "$STAGE2_ADAPTER_DATASET_MAX_CLIPS")
  fi
  ...
  if [[ -n "${STAGE2_ADAPTER_STARTUP_PREFLIGHT_BATCHES:-}" ]]; then
    args+=(--startup-preflight-batches "$STAGE2_ADAPTER_STARTUP_PREFLIGHT_BATCHES")
  fi
...
log "Dataset smoke cap: max_samples=${STAGE2_ADAPTER_MAX_SAMPLES:-0} max_clips=${STAGE2_ADAPTER_DATASET_MAX_CLIPS:-0}"
...
log "Startup preflight batches: ${STAGE2_ADAPTER_STARTUP_PREFLIGHT_BATCHES:-<config default>}"
