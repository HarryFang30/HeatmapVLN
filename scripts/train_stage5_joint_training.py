#!/usr/bin/env python3
"""
Stage 5: 联合训练 (128→224)
训练：所有模块（history + future + mlp + temporal + encoder）
"""
...
def main():
    parser = argparse.ArgumentParser(description='Stage 5: 联合训练')
    ...
    stage_cfg = cfg['training']['stages'][4]  # stage5_joint_training
    stage_name = stage_cfg['name']

    logger.info(f"\n{'='*80}")
    logger.info(f"Stage 5: 联合训练 (128→224) - 双头联合优化，最终高分辨率")
    logger.info(f"{'='*80}\n")
    ...
    if rank == 0 and cfg['log'].get('use_tensorboard', False):
        ...
        tb_writer = SummaryWriter(log_dir=str(tb_dir / f'stage5_{timestamp}'))
        logger.info(f"📊 TensorBoard enabled: {tb_dir / f'stage5_{timestamp}'}")

    best_val_loss = float('inf')
    stage_idx = 4  # ⭐ Stage 5 对应 idx=4
    ...
    if rank == 0:
        ...
        logger.info(f"\n✅ 全部训练完成! Best val loss: {best_val_loss:.4f}")
        logger.info(f"最佳模型保存在: {cfg['log']['out_dir']}/{stage_name}_best.pth\n")
