#!/usr/bin/env bash
# Website-submit entry point for the formal 4-GPU PPA two-stage run.
#
...
PPA_STAGE1_CONFIG="${PPA_STAGE1_CONFIG:-${PPA_REPO_ROOT}/configs/ppa_stage1_map_pretrain_4gpu.yaml}"
PPA_STAGE2_CONFIG="${PPA_STAGE2_CONFIG:-${PPA_REPO_ROOT}/configs/ppa_stage2_joint_4gpu.yaml}"
PPA_CONTRACT_CHECKER="${PPA_CONTRACT_CHECKER:-${PPA_REPO_ROOT}/scripts/tools/validate_ppa_4gpu_contract.py}"

PPA_GPU_DEVICES="${PPA_GPU_DEVICES:-0,1,2,3}"
PPA_MASTER_ADDR="${PPA_MASTER_ADDR:-127.0.0.1}"
PPA_STAGE1_MASTER_PORT="${PPA_STAGE1_MASTER_PORT:-29641}"
PPA_STAGE2_MASTER_PORT="${PPA_STAGE2_MASTER_PORT:-29642}"
PPA_STAGE1_EPOCHS="${PPA_STAGE1_EPOCHS:-4}"
PPA_STAGE2_EPOCHS="${PPA_STAGE2_EPOCHS:-4}"
PPA_NUM_WORKERS="${PPA_NUM_WORKERS:-4}"
PPA_PREFETCH_FACTOR="${PPA_PREFETCH_FACTOR:-2}"
# Warm the AFS-backed Python/module pages once before four ranks import them.
...
[[ "$PPA_GPU_DEVICES" =~ ^[0-9]+,[0-9]+,[0-9]+,[0-9]+$ ]] \
  || die "PPA_GPU_DEVICES must contain exactly four comma-separated IDs"
...
if [[ "$PPA_PREWARM_IMPORTS" == "1" ]]; then
  ...
  log "prewarming scripts.train once before four-rank launch"
  if ! (
    cd "$PPA_REPO_ROOT"
...
run_stage() {
  ...
  log "starting ${label}: world=4 per_rank_batch=1 accum=2 effective_batch=8"
