...
def build_future_heatmap_loss_fn(cfg: dict, device: torch.device):
    ...
    A temporal tube has no unique point coordinate or mutually exclusive
    panorama view.  Fail closed instead of silently enabling those Past-map
    objectives through a shared config.
    """
    ...
    if violations:
        raise ValueError(
            "Future trajectory tube cannot enable point/single-view losses "
            f"or probability fallback: {violations}"
        )
