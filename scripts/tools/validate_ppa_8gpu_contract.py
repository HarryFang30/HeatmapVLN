...
EXPECTED_PER_RANK_BATCH = 1
EXPECTED_GRAD_ACCUMULATION = 1
EXPECTED_EFFECTIVE_GLOBAL_BATCH = 8

EXPECTED_STAGE_CONTRACTS: dict[str, dict[str, Any]] = {
    "stage1_map_pretrain": {
        "train_action": False,
        "learning_rate": 2.0e-5,
        "past_plan_action_future_lr": 1.0e-4,
        "past_plan_action_bridge_lr": 2.0e-5,
        "heatmap_coarse_lr": 2.0e-5,
        ...
        "preserve_weight": 0.0,
        "delta_z_weight": 0.0,
        "best_metric": "val_future_soft_iou",
        "best_mode": "max",
    },
    "stage2_joint": {
        "train_action": True,
        "learning_rate": 1.0e-5,
        "past_plan_action_future_lr": 5.0e-5,
        "past_plan_action_bridge_lr": 2.0e-5,
...
def validate_config(schema_raw: str, config_raw: str, expected_stage: str) -> dict[str, Any]:
    ...
    stage = stages[0]
    expected = EXPECTED_STAGE_CONTRACTS[expected_stage]
    ...
    _expect(cfg["data"]["num_workers"], 2, "configured workers per rank")
    _expect(cfg["data"]["prefetch_factor"], 2, "configured prefetch factor")
    ...
    _expect(
        trajectory["enable_trajectory_augmentation"],
        False,
        "canonical action/Future geometry",
    )
    _expect(trajectory["single_view_rgb_input"], True, "single-view Past input")
    _expect(trajectory["future_heatmap"]["enabled"], True, "Future targets")
    ...
    _expect(stage["train_action"], expected["train_action"], "action training")
    _expect(stage["train_history"], True, "History training")
    _expect(stage["train_future"], True, "Future training")
    _expect(stage["train_lm"], False, "language-model training")
    _expect(stage["train_system2_sft"], False, "System2 SFT training")
    _expect(stage["trajectory_sequence_mode"], "first_only", "trajectory sequence mode")
    _expect(stage["strict_trainable_modules"], True, "strict trainable scope")
    _expect(
        stage["require_complete_internnav_system1"],
        True,
        "complete frozen InternNav System1",
    )

    heatmap = cfg["model"]["heatmap"]
    _expect(heatmap["input_mode"], "internnav_single_view", "Past input mode")
    _expect(heatmap["feature_source"], "vit_only", "Past feature source")
    _expect(heatmap["heatmap_trains_backbone"], False, "frozen visual backbone")
    _expect(
        cfg["model"]["action_head"]["nextdit"]["heatmap_control"]["enabled"],
        False,
        "legacy per-layer heatmap control",
    )
    ppa = cfg["model"]["past_plan_action"]
    _expect(ppa["enabled"], True, "PPA chain")
    _expect(ppa["schema_version"], "past-plan-action-v1", "PPA schema")
    _expect(ppa["plan_dim"], 768, "Plan dimension")
    _expect(ppa["memory_dim"], 256, "History memory dimension")

    optim = cfg["optim"]
    for field in (
        "learning_rate",
        "past_plan_action_future_lr",
        "past_plan_action_bridge_lr",
        "heatmap_coarse_lr",
        "heatmap_fine_lr",
    ):
        _expect(float(optim[field]), expected[field], f"optim.{field}")
    _expect(optim["amp"], "bf16", "AMP mode")
    ...
    for field in (
        ...
        _expect(float(loss[field]), expected[field], f"loss.{field}")
    _expect(
        float(loss["future_heatmap"]["lambda_coord"]),
        0.0,
        "Future tube coordinate loss",
    )
    _expect(
        loss["future_heatmap"]["allow_probability_fallback"],
        False,
        "Future raw-logit loss contract",
    )
    _expect(
        cfg["validation"]["save_best_metric"],
        expected["best_metric"],
        "best checkpoint metric",
    )
    _expect(
        cfg["validation"]["save_best_mode"],
        expected["best_mode"],
    ...
    _expect(
        effective_batch,
        EXPECTED_EFFECTIVE_GLOBAL_BATCH,
