"""
Habitat client for the counterfactual candidate-support audit.
...
The deployed arm remains frozen native InternNav by default. At each System2
trajectory call, the client receives paired native/control candidates, reduces
them to exact TreatmentSpecs, shadow-executes unique local treatments, and
writes compact features plus vector-valued outcomes for later identifiability
probes. Adapted for habitat-lab 0.1.7 (YACS config).
"""
...
_PROJECT_ROOT = _LOCKED_HEATMAP_REPO
...
from control_result_contract import (
    add_control_counters,
    empty_control_counters,
    summarize_episode_rows,
    validate_episode_control_summary,
    validate_control_rpc_response,
)

# Load the pure VO protocol module directly so the default GT-pose evaluator
# does not execute ``src.vo.__init__`` or import AMB3R-related packages.
_VO_RPC_PROTOCOL_PATH = _PROJECT_ROOT / "src" / "vo" / "rpc_protocol.py"
_VO_RPC_PROTOCOL_SPEC = importlib.util.spec_from_file_location(
    "_heatmapvln_vo_rpc_protocol",
    _VO_RPC_PROTOCOL_PATH,
)
if _VO_RPC_PROTOCOL_SPEC is None or _VO_RPC_PROTOCOL_SPEC.loader is None:
    raise ImportError(f"Could not load VO RPC protocol from {_VO_RPC_PROTOCOL_PATH}")
_vo_rpc_protocol = importlib.util.module_from_spec(_VO_RPC_PROTOCOL_SPEC)
sys.modules[_VO_RPC_PROTOCOL_SPEC.name] = _vo_rpc_protocol
_VO_RPC_PROTOCOL_SPEC.loader.exec_module(_vo_rpc_protocol)
AMB3R_VO_FRONT_BLOB_NAME = _vo_rpc_protocol.AMB3R_VO_FRONT_BLOB_NAME
AMB3R_VO_INGEST_METHOD = _vo_rpc_protocol.AMB3R_VO_INGEST_METHOD
AMB3R_VO_POSE_PROVIDER = _vo_rpc_protocol.AMB3R_VO_POSE_PROVIDER
AMB3R_VO_QUERY_METHOD = _vo_rpc_protocol.AMB3R_VO_QUERY_METHOD
AMB3R_VO_RESET_METHOD = _vo_rpc_protocol.AMB3R_VO_RESET_METHOD
AMB3R_VO_RPC_PROTOCOL_VERSION = _vo_rpc_protocol.AMB3R_VO_RPC_PROTOCOL_VERSION
HABITAT_GT_POSE_PROVIDER = _vo_rpc_protocol.HABITAT_GT_POSE_PROVIDER
VOFrameLedger = _vo_rpc_protocol.VOFrameLedger
model_pose_fields_from_query = _vo_rpc_protocol.model_pose_fields_from_query
native_front_rgb = _vo_rpc_protocol.native_front_rgb
validate_ingest_response = _vo_rpc_protocol.validate_ingest_response
validate_reset_response = _vo_rpc_protocol.validate_reset_response
vo_rpc_payload = _vo_rpc_protocol.vo_rpc_payload
...
# InternNav's official evaluator keeps the 640x480 sensor resolution for the
# optional conversational look-down image.  The model server derives the
# separate 224x224 System1 tensor from this same native frame.
NATIVE_LOOKDOWN_SENSOR_SIZE = (640, 480)
...
def _resolve_eval_paths(args, split: str) -> None:
...
def _load_progress(
    ...
    *,
    ...
    expected_control_artifacts: dict[str, str] | None = None,
    expected_history_pose_source: str | None = None,
    ...
    with open(progress_file) as f:
        for line in f:
            if not line.strip():
                continue
            res = json.loads(line)
            if expected_history_pose_source is not None:
                recorded_pose_source = res.get(
                    "history_pose_source", HABITAT_GT_POSE_PROVIDER
                )
                if recorded_pose_source != expected_history_pose_source:
                    raise ValueError(
                        "Progress history pose source mismatch: "
                        f"recorded={recorded_pose_source!r}, "
                        f"expected={expected_history_pose_source!r}"
                    )
            ...
            if expected_control_mode is not None:
                if expected_control_artifacts is None:
                    raise ValueError(
                        "expected_control_artifacts is required with expected_control_mode"
                    )
                validate_episode_control_summary(
                    res,
                    expected_mode=expected_control_mode,
                    expected_artifacts=expected_control_artifacts,
                )
...
def build_habitat_config(args):
    ...
    if dataset_split not in {"train", "val_seen", "val_unseen"}:
        raise ValueError(f"unsupported Habitat dataset split {dataset_split!r}")
    cfg.DATASET.SPLIT = dataset_split
...
def _capture_history_record(env, image_size: tuple[int, int], capture_step: int) -> dict[str, Any]:
    """Capture RGB and pose from one unchanged simulator state."""
    if int(capture_step) < 0:
        raise ValueError("capture_step must be non-negative")
    c2w = get_agent_cam2world(env)
    views = capture_panoramic_views(env, image_size=image_size)
    return {
        "views": views,
        "c2w": c2w,
...
def _sample_history_records(
    ...
    """Use exactly the native history indices while retaining synchronized metadata."""
    if num_history <= 0:
        return []
    if len(records) <= num_history:
        selected = list(records)
    else:
        indices = np.unique(
            np.linspace(0, len(records) - 1, num_history, dtype=np.int32)
        ).tolist()
        selected = [records[index] for index in indices]
    required_keys = {"views", "c2w", "capture_step"}
    optional_keys = {"system2_call_index", "vo_frame_id"}
    for record in selected:
        ...
        if missing or unknown:
            raise ValueError(
                "Malformed history record keys: "
                f"missing={missing}, unknown={unknown}, actual={sorted(keys)}"
            )
        if "system2_call_index" in record:
            call_index = record["system2_call_index"]
            if (
                isinstance(call_index, (bool, np.bool_))
                or not isinstance(call_index, (int, np.integer))
                or int(call_index) < -1
            ):
                raise ValueError(
                    "history system2_call_index must be an integer >= -1; "
                    f"got {call_index!r}"
                )
        if "vo_frame_id" in record:
            frame_id = record["vo_frame_id"]
            if (
                ...
                raise ValueError(
                    "history vo_frame_id must be a non-negative integer; "
                    f"got {frame_id!r}"
                )
    return selected
...
def _rpc_blob_from_native_rgb(name: str, rgb: np.ndarray, quality: int) -> dict:
    """Encode native sensor RGB without applying the model's 384px resize."""
    from vla_rpc.core.image import encode_rgb_to_jpeg

    arr = native_front_rgb(rgb)
    return {
        "name": name,
        "data": encode_rgb_to_jpeg(arr, quality=int(quality)),
        "mime_type": "image/jpeg",
        "height": int(arr.shape[0]),
        "width": int(arr.shape[1]),
    }
...
class _OnlineVORPCBridge:
    """Episode-local client state for a separate mutable AMB3R-VO server."""

    def __init__(self, client: Any, *, jpeg_quality: int) -> None:
        self.client = client
    ...
    def ingest_horizontal_observation(
        self,
        ...
        *,
        ...
        if not is_new:
            return frame_id
        raw = _extract_rgb_array(observations)
        if raw is None:
            raise RuntimeError(
                "Could not extract native horizontal front RGB for AMB3R VO"
            )
        rgb = native_front_rgb(raw)
        response = self._infer(
    ...
    def query_model_pose_fields(
        ...
        *,
        ...
        current_frame_id = int(current_record["vo_frame_id"])
        history_frame_ids = [
            int(record["vo_frame_id"]) for record in prompt_records
        ]
        response = self._infer(
...
def _agent_body_pose(env) -> np.ndarray:
    state = env._sim.get_agent(0).get_state()
    pose = np.eye(4, dtype=np.float32)
    pose[:3, :3] = quaternion.as_rotation_matrix(state.rotation).astype(np.float32)
    pose[:3, 3] = np.asarray(state.position, dtype=np.float32)
    return pose


def _decode_candidate_audit_blob(
    response: dict[str, Any],
    response_blobs: list[Any],
) -> dict[str, np.ndarray]:
    manifest = response.get("candidate_audit")
    if not isinstance(manifest, dict):
        raise RuntimeError("trajectory response omitted candidate_audit manifest")
    expected_contract = {
    ...
    if mismatches:
        raise RuntimeError(f"candidate export contract mismatch: {mismatches}")
    matches = [blob for blob in response_blobs if blob.name == CANDIDATE_BLOB_NAME]
    if len(matches) != 1 or len(response_blobs) != 1:
        raise RuntimeError(
            "candidate trajectory response must contain exactly one audit blob: "
            f"all={[blob.name for blob in response_blobs]}"
        )
    blob = matches[0]
    payload = bytes(blob.data)
    if blob.mime_type != "application/x-npz":
        raise RuntimeError(f"unexpected candidate blob MIME type {blob.mime_type!r}")
    if len(payload) != int(manifest.get("blob_bytes", -1)):
        raise RuntimeError("candidate blob byte count differs from manifest")
    if sha256_bytes(payload) != manifest.get("blob_sha256"):
        raise RuntimeError("candidate blob SHA256 differs from manifest")

    with np.load(io.BytesIO(payload), allow_pickle=False) as archive:
        arrays = {name: np.ascontiguousarray(archive[name]) for name in archive.files}
    actual = compact_array_manifest(arrays)
    if actual.get("arrays") != manifest.get("arrays"):
        raise RuntimeError("decoded candidate arrays differ from server manifest")
    required = {
        "native_trajectories",
        "heatmap_trajectories",
    ...
    if missing:
        raise RuntimeError(f"candidate blob omitted required arrays: {missing}")
    if arrays["native_trajectories"].shape != arrays["heatmap_trajectories"].shape:
        raise RuntimeError("paired candidate arrays have different shapes")
    return arrays


def _single_trajectory_to_native_actions(trajectory: np.ndarray) -> list[int]:
    """Apply InternNav's single-sample postprocessor without client-side Torch.

    The Habitat-only environment intentionally ships a minimal ``torch`` shim;
    model-side tensor work belongs to the RPC server.  This is the exact
    single-sample specialization of ``internnav.model.utils.vln_utils``:
    unnormalize XY by four, cumulatively reconstruct the path, then use the
    native 0.25 m / 15 degree / lookahead-4 discretizer.
    """

    value = np.asarray(trajectory, dtype=np.float32)
    if value.ndim != 2 or value.shape[0] < 1 or value.shape[1] != 3:
        raise ValueError(f"trajectory must have shape [T,3], got {value.shape}")
    if not np.isfinite(value).all():
        raise ValueError("trajectory contains non-finite values")
    unnormalized = np.ascontiguousarray(value).copy()
    unnormalized[:, :2] /= 4.0
    path = reconstruct_xy_from_delta(unnormalized[None, ...])[0]
    return [
        int(action)
        for action in _trajectory_to_discrete_actions_close_to_goal(path)
    ]


def _oscillation_from_actions(actions: list[int], *, window: int = 8) -> bool:
    recent = [int(action) for action in actions[-int(window) :]]
    if len(recent) < 4:
        return False
    turn_pairs = {(ActionCode.LEFT, ActionCode.RIGHT), (ActionCode.RIGHT, ActionCode.LEFT)}
    alternating = sum(
        (left, right) in turn_pairs for left, right in zip(recent, recent[1:])
    )
    return alternating >= 3


def _collect_candidate_audit_state(
    *,
    ...
    current_record: dict[str, Any],
    prompt_records: list[dict[str, Any]],
    executed_history_records: list[dict[str, Any]],
    episode_start_body_pose: np.ndarray,
    goal_position: np.ndarray,
    executed_action_prefix: list[int],
    visited_body_poses: list[np.ndarray],
    success_radius_m: float,
) -> dict[str, Any]:
    arrays = _decode_candidate_audit_blob(response, response_blobs)
    candidate_set = build_candidate_set(
        arrays["native_trajectories"],
        heatmap_trajectories=arrays["heatmap_trajectories"],
        trajectory_to_actions=_single_trajectory_to_native_actions,
        prefix_lengths=(1, 2, 3, 4),
        include_replan_now=True,
        max_steps=MAX_STEPS,
        max_local_steps=MAX_LOCAL_STEPS,
    )

    current_body_pose = np.asarray(backend.get_pose(), dtype=np.float32)
    route_observation = route_tracker.observe(current_body_pose[:3, 3])
    local_values = [
        evaluate_local_treatment(
            backend,
            aggregated.treatment,
            start_pose=current_body_pose,
            route_tracker=route_tracker,
            route_progress_m=route_observation.progress_m,
            goal_position=goal_position,
            older_poses=(
                np.stack(visited_body_poses)
                if visited_body_poses
                else np.empty((0, 4, 4), dtype=np.float32)
            ),
            success_radius_m=success_radius_m,
        )
        for aggregated in candidate_set.treatments
    ]
    local_outcomes = [value.to_dict() for value in local_values]
    max_trace_length = MAX_LOCAL_STEPS + 1
    padded_pose_traces = []
    for value in local_values:
        trace = np.asarray(value.pose_trace, dtype=np.float32)
        if not 1 <= len(trace) <= max_trace_length:
            raise RuntimeError(f"invalid local pose-trace length {len(trace)}")
        if len(trace) < max_trace_length:
            trace = np.concatenate(
                [
                    trace,
                    np.repeat(
                        trace[-1:], max_trace_length - len(trace), axis=0
                    ),
                ],
                axis=0,
            )
        padded_pose_traces.append(trace)

    arrays = dict(arrays)
    arrays.update(
        {
            "client_current_body_pose": current_body_pose,
            "client_episode_start_body_pose": np.asarray(
                episode_start_body_pose, dtype=np.float32
            ),
            "client_goal_position": np.asarray(goal_position, dtype=np.float32),
            "client_reference_path": np.asarray(route_tracker.points, dtype=np.float32),
            "client_executed_action_prefix": np.asarray(
                executed_action_prefix, dtype=np.int8
            ),
            "client_visited_body_poses": (
                np.stack(visited_body_poses).astype(np.float32)
                if visited_body_poses
                else np.empty((0, 4, 4), dtype=np.float32)
            ),
            "client_full_history_c2w": (
                np.stack([record["c2w"] for record in executed_history_records]).astype(
                    np.float32
                )
                if executed_history_records
                else np.empty((0, 4, 4), dtype=np.float32)
            ),
            "client_full_history_capture_steps": np.asarray(
                [record["capture_step"] for record in executed_history_records],
                dtype=np.int32,
            ),
            "client_full_history_call_indices": np.asarray(
                [
                    record.get("system2_call_index", -1)
                    for record in executed_history_records
                ],
                dtype=np.int32,
            ),
            "local_treatment_id_ascii": np.asarray(
                [value.treatment_id.encode("ascii") for value in local_values],
                dtype="S64",
            ),
            "local_endpoint_poses": np.stack(
                [value.endpoint_pose for value in local_values]
            ).astype(np.float32),
            "local_pose_traces": np.stack(padded_pose_traces).astype(np.float32),
            "local_pose_trace_lengths": np.asarray(
                [len(value.pose_trace) for value in local_values], dtype=np.int8
            ),
        }
    )
    state_key = (
        f"{dataset_split}:{scene_id}:{int(episode_id)}:"
        f"s2:{int(system2_call_index)}:step:{int(step_id)}"
    )
    export_manifest = dict(response["candidate_audit"])
    export_manifest.pop("arrays", None)
    distance_to_goal = float(
        np.linalg.norm(current_body_pose[:3, 3] - np.asarray(goal_position))
    )
    record = {
        "dataset_split": str(dataset_split),
        "state_distribution": f"{response.get('deployment_arm')}_on_policy",
        "scene_id": str(scene_id),
        "episode_id": int(episode_id),
        "system2_call_index": int(system2_call_index),
        "step_id": int(step_id),
        "instruction": str(instruction),
        "candidate_export": export_manifest,
        "candidate_set": candidate_set.to_dict(),
        "candidate_count_sensitivity": {
            "native": candidate_count_sensitivity(candidate_set, arm="native"),
            "heatmap_control": candidate_count_sensitivity(
                candidate_set, arm="heatmap_control"
            ),
        },
        "local_outcomes": local_outcomes,
        "local_outcome_semantics": {
            "scope": "open_loop_unique_treatment",
            "authoritative_success": False,
            "goal_distance": "euclidean_auxiliary_only",
            "system1_stop": "replan",
            "system2_stop": "episode_terminal",
            "max_executable_actions": MAX_LOCAL_STEPS,
        },
        "continuation_outcomes": {
            "status": "not_run_in_local_collection_pass",
            "label_policy": "one_deviation_then_frozen_native_pi0",
            "required_horizons_system2_cycles": [1, 3, 5, "episode_end"],
            "replay_source": "runtime_snapshot_and_compact_arrays",
            "fixed_current_system2_output_for_candidate_chunk": True,
            "rerun_system2_independently_after_candidate_chunk": True,
        },
        "runtime_snapshot": {
            "schema": "candidate-runtime-replay-snapshot-v1",
            "closed_loop_continuation_mode": "deterministic_episode_prefix_replay",
            "executed_action_prefix_length": len(executed_action_prefix),
            "local_action_queue": [],
            "forward_action_count": 0,
            "system2_calls_completed": int(system2_call_index) + 1,
            "current_capture_step": int(current_record["capture_step"]),
            "history_capture_steps": [
                int(record["capture_step"]) for record in prompt_records
            ],
            "full_history_capture_steps": [
                int(record["capture_step"]) for record in executed_history_records
            ],
            "full_history_call_indices": [
                int(record.get("system2_call_index", -1))
                for record in executed_history_records
            ],
            "pixel_goal": response.get("pixel_goal"),
            "llm_output": response.get("llm_output"),
            "native_first_output": response.get("native_first_output"),
            "native_lookdown_turns": response.get("native_lookdown_turns"),
            "sampling": response.get(HEATMAPVLN_RPC_SAMPLING_FIELD),
            "noise_sha256": response["candidate_audit"].get(
                "initial_noise_sha256"
            ),
            "anti_deadlock": bool(response.get("anti_deadlock", False)),
        },
        "route_state": route_tracker.state_dict(),
        "state_strata": {
            "primary_native_distribution": response.get("deployment_arm") == "native",
            "near_goal": distance_to_goal <= 5.0,
            "recovery": float(route_observation.offpath_m) >= 0.75,
            "oscillation": _oscillation_from_actions(executed_action_prefix),
        },
        "current_euclidean_goal_distance_m": distance_to_goal,
        "current_route_offpath_m": float(route_observation.offpath_m),
        "current_route_progress_m": float(route_observation.progress_m),
    }
    return writer.commit(state_key=state_key, record=record, arrays=arrays)
...
def _rpc_plan_panoramic(
    ...
    *,
    ...
    current_views: dict[str, Image.Image],
    history_panoramas: list[dict[str, Image.Image]],
    current_c2w: np.ndarray | None,
    history_c2w: list[np.ndarray] | None,
    current_capture_step: int,
    ...
    expected_control_artifacts: dict[str, str],
    oracle_system2: dict[str, Any] | None = None,
    external_pose_fields: dict[str, Any] | None = None,
) -> tuple[dict, dict[str, int], list[Any]]:
    history_count = len(history_panoramas)
    if len(history_capture_steps) != history_count:
        raise ValueError("RPC history RGB/capture-step lengths must match")
    if external_pose_fields is None:
        if current_c2w is None or history_c2w is None:
            raise ValueError("Habitat GT pose provider requires current/history c2w")
        if len(history_c2w) != history_count:
            raise ValueError("RPC history RGB/GT-pose lengths must match")
    else:
        if current_c2w is not None or history_c2w is not None:
            raise ValueError("External pose provider must not receive Habitat c2w")
        if external_pose_fields.get("pose_provider") != AMB3R_VO_POSE_PROVIDER:
            raise ValueError("Unsupported external pose provider fields")
        leaked = sorted(
            {"current_c2w", "history_c2w"}.intersection(external_pose_fields)
        )
        if leaked:
            raise ValueError(
                f"External pose provider fields leaked privileged c2w: {leaked}"
            )
    if int(current_capture_step) < 0 or any(
        int(step) < 0 or int(step) > int(current_capture_step)
        for step in history_capture_steps
    ):
        raise ValueError("RPC history capture steps must be causal")
    blobs = []
    ...
    payload = {
        ...
        "history_capture_steps": [int(step) for step in history_capture_steps],
        "history_age_steps": [
            int(current_capture_step) - int(step) for step in history_capture_steps
        ],
    ...
    if external_pose_fields is None:
        payload.update(
            {
                "pose_provider": HABITAT_GT_POSE_PROVIDER,
                "current_c2w": np.asarray(current_c2w, dtype=np.float32).tolist(),
                "history_c2w": [
                    np.asarray(pose, dtype=np.float32).tolist()
                    for pose in history_c2w
                ],
            }
        )
    else:
        payload.update(external_pose_fields)
    ...
    if response.get("control_proto_v") != "heatmap-control-eval-v1":
        raise RuntimeError(
            f"Control RPC response mismatch: {response.get('control_proto_v')!r}"
        )
    ...
    control_accounting = validate_control_rpc_response(
        response,
        expected_mode=expected_control_mode,
        expected_history_count=history_count,
        expected_artifacts=expected_control_artifacts,
    )
    if response.get("candidate_export_proto_v") != CANDIDATE_EXPORT_PROTO_VERSION:
        raise RuntimeError(
            "candidate export response protocol mismatch: "
            f"{response.get('candidate_export_proto_v')!r}"
        )
    return response, control_accounting, list(response_blobs)
...
def run_eval_rpc_panoramic(args):
    ...
    print(f"expected_control_mode={args.expected_control_mode}")
    control_artifacts = {
    ...
    print(f"expected_control_artifacts={control_artifacts}")
    print(
        "rpc_sampling="
    ...
    if info is None:
        raise RuntimeError("candidate audit server did not return ServerInfo")
    ...
    expected_model_version = (
        ...
        f"deployment-{args.candidate_audit_deployment_arm}"
    )
    if info.model_version != expected_model_version:
        raise RuntimeError(
            "candidate audit deployment arm mismatch: "
            f"server={info.model_version!r} expected={expected_model_version!r}"
        )
    if CANDIDATE_EXPORT_PROTO_VERSION not in set(info.supported_formats):
        raise RuntimeError(
            "candidate export capability missing from server: "
            f"formats={list(info.supported_formats)!r}"
        )

    vo_bridge: _OnlineVORPCBridge | None = None
    if args.history_pose_source == AMB3R_VO_POSE_PROVIDER:
        ...
        if vo_info is None:
            raise RuntimeError("AMB3R VO RPC server did not return ServerInfo")
        if vo_info.version != AMB3R_VO_RPC_PROTOCOL_VERSION:
            raise RuntimeError(
                "AMB3R VO RPC protocol mismatch: "
                f"server={vo_info.version!r} "
                f"expected={AMB3R_VO_RPC_PROTOCOL_VERSION!r}"
            )
        if vo_info.model_version != "amb3r-vo-da3-online":
            raise RuntimeError(
                "Unexpected AMB3R VO model version: "
                f"{vo_info.model_version!r}"
            )
        if "json+jpeg" not in set(vo_info.supported_formats):
            raise RuntimeError(
                "AMB3R VO RPC server lacks json+jpeg support: "
                f"{list(vo_info.supported_formats)!r}"
            )
        vo_bridge = _OnlineVORPCBridge(
            vo_client,
            jpeg_quality=args.amb3r_vo_rpc_jpeg_quality,
        )
        print(
            "AMB3R VO RPC: "
            f"server={args.amb3r_vo_rpc_server} "
            f"protocol={vo_info.version} model={vo_info.model_version}",
            flush=True,
        )
    elif str(args.amb3r_vo_rpc_server).strip():
        print(
            "WARNING: --amb3r_vo_rpc_server is ignored because "
            "--history_pose_source=habitat_gt_c2w",
            flush=True,
        )
    ...
    audit_root = Path(args.candidate_audit_root).expanduser().resolve()
    allowed_root = LOCAL_FJL_ROOT.expanduser().resolve()
    try:
        audit_root.relative_to(allowed_root)
    except ValueError as exc:
        raise ValueError(
            f"candidate_audit_root must stay under {allowed_root}: {audit_root}"
        ) from exc
    audit_writer = AuditShardWriter(
        audit_root,
        shard_id=int(args.candidate_audit_shard_id),
        max_bytes=int(round(float(args.candidate_audit_max_gb_per_shard) * 1_000_000_000)),
    )
    audit_backend = dagger_api.HabitatShadowBackend(env._sim)
    print(
        "candidate_audit="
        f"root={audit_root} shard={audit_writer.shard_id} "
        f"resumed_records={audit_writer.record_count} "
        f"max_bytes={audit_writer.max_bytes}",
        flush=True,
    )
    ...
    sucs, spls, oss, nes, done_set = _load_progress(
        ...
        expected_history_pose_source=str(args.history_pose_source),
    )
    ...
    while True:
        ...
        if vo_bridge is not None:
            vo_session_id = f"{scene_id}/{episode_id:04d}"
            vo_bridge.reset_episode(
                vo_session_id,
                max_frames=int(max_steps_per_episode) + 1,
            )

        audit_goal_position = np.asarray(episode.goals[0].position, dtype=np.float32)
        audit_reference_path = _episode_reference_path(episode) or [
            audit_goal_position.tolist()
        ]
        audit_route_tracker = dagger_api.MonotonicRouteTracker(audit_reference_path)
        audit_episode_start_body_pose = _agent_body_pose(env)
        audit_route_tracker.observe(audit_episode_start_body_pose[:3, 3])
        audit_executed_actions: list[int] = []
        audit_visited_body_poses: list[np.ndarray] = []
        ...
        system2_calls = 0
        trajectory_calls = 0
        episode_control = empty_control_counters()
        step_id = 0
        done = False
        ...
        while (not done) and (step_id < max_steps_per_episode):
            ...
            if vo_bridge is not None:
                # This is the task observation returned by reset or a real
                # navigation action. Temporary look-down capture does not
                # replace it, so pitched RGB is never ingested by VO.
                current_vo_frame_id = vo_bridge.ingest_horizontal_observation(
                    observations,
            ...
            if stop_result is not None:
                ...
                audit_visited_body_poses.append(_agent_body_pose(env))
                audit_executed_actions.append(int(ActionCode.STOP))
                _record_post_action_step(
                    step_recorder,
                    env,
                    step_id=new_step_id,
            ...
            if local_actions:
                history_record = _capture_history_record(env, image_size, step_id)
                if current_vo_frame_id is not None:
                    history_record["vo_frame_id"] = current_vo_frame_id
                history_record["system2_call_index"] = max(system2_calls - 1, -1)
                current_views = history_record["views"]
                executed_history_records.append(history_record)
                ...
                audit_visited_body_poses.append(_agent_body_pose(env))
                observations, done = _apply_habitat_action(env, action)
                audit_executed_actions.append(int(action))
                if before is not None:
                    print(
                        f"  [debug] executed local action={int(action)} {before} -> {_env_trace_summary(env)}",
                        flush=True,
            ...
            current_record = _capture_history_record(env, image_size, step_id)
            if current_vo_frame_id is not None:
                current_record["vo_frame_id"] = current_vo_frame_id
            current_record["system2_call_index"] = system2_calls
            current_views = current_record["views"]
            prompt_records = _sample_history_records(
                executed_history_records,
                num_history,
            )
            prompt_history = [record["views"] for record in prompt_records]
            ...
            # Preserve InternNav's native VLM look-down resolution.  The RPC
            # server resizes a copy to traj_image_size for asynchronous
            # NextDiT, avoiding a lossy 224 -> VLM upsample in the ↓ turn.
            lookdown_img = capture_lookdown_view(
                env,
                image_size=NATIVE_LOOKDOWN_SENSOR_SIZE,
            )
            executed_history_records.append(current_record)
            system2_calls += 1
            oracle_system2 = None
            if bool(getattr(args, "oracle_system2", False)):
            ...
            response, control_accounting, candidate_response_blobs = _rpc_plan_panoramic(
                client,
                instruction=instruction,
                current_views=current_views,
                history_panoramas=prompt_history,
                current_c2w=(
                    None if external_pose_fields is not None else current_record["c2w"]
                ),
                history_c2w=(
                    None
                    if external_pose_fields is not None
                    else [record["c2w"] for record in prompt_records]
                ),
                current_capture_step=current_record["capture_step"],
                history_capture_steps=[
                    record["capture_step"] for record in prompt_records
                ],
                ...
                external_pose_fields=external_pose_fields,
            )
            add_control_counters(episode_control, control_accounting)
            if response.get("kind") == "trajectory":
                audit_row = _collect_candidate_audit_state(
                    writer=audit_writer,
                    backend=audit_backend,
                    route_tracker=audit_route_tracker,
                    response=response,
                    response_blobs=candidate_response_blobs,
                    ...
                    current_record=current_record,
                    prompt_records=prompt_records,
                    executed_history_records=list(executed_history_records),
                    episode_start_body_pose=audit_episode_start_body_pose,
                    goal_position=audit_goal_position,
                    executed_action_prefix=list(audit_executed_actions),
                    visited_body_poses=list(audit_visited_body_poses),
                    success_radius_m=float(args.candidate_audit_success_radius_m),
                )
                print(
                    "  [candidate-audit] "
                    f"state={audit_row['state_key']} "
                    f"unique={audit_row['candidate_set']['unique_treatment_count']} "
                    f"array_bytes={audit_row['array_file_bytes']}",
                    flush=True,
                )
            llm_output = response.get("llm_output", "")
            ...
            audit_visited_body_poses.append(_agent_body_pose(env))
            observations, done = _apply_habitat_action(env, first_action)
            audit_executed_actions.append(int(first_action))
            if before is not None:
                print(
    ...
    audit_manifest = audit_writer.seal(
        extra={
            "collector": "counterfactual-candidate-support-and-identifiability-audit",
            "dataset_split": str(args.dataset_split),
            "state_distribution": f"{args.candidate_audit_deployment_arm}_on_policy",
            ...
            "frozen_heatmap_checkpoint_sha256": str(
                args.expected_heatmap_checkpoint_sha256
            ),
            "control_ema_checkpoint_sha256": str(
                args.expected_control_checkpoint_sha256
            ),
            "native_model_manifest_sha256": str(
                args.expected_native_model_manifest_sha256
            ),
        }
    )
    print(
        "[candidate-audit] sealed "
        f"records={audit_manifest['record_count']} "
        f"array_bytes={audit_manifest['array_bytes']} "
        f"manifest={audit_writer.manifest_path}",
        flush=True,
    )
    env.close()
    client.close()
    ...
    progress_rows = [
        json.loads(line)
        for line in Path(progress_file).read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    control_summary = summarize_episode_rows(
        progress_rows,
        expected_mode=args.expected_control_mode,
        expected_artifacts=control_artifacts,
    )
    if int(control_summary["control_episode_count"]) != int(final_result["total_episodes"]):
        raise RuntimeError(
            "RPC control-accounting episode count differs from navigation metrics: "
            f"control={control_summary['control_episode_count']} "
            f"navigation={final_result['total_episodes']}"
        )
    final_result.update(
        {
            "rpc_protocol": HEATMAPVLN_RPC_PROTOCOL_VERSION,
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect paired candidate-support audit states in Habitat"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument("--output_path", type=str, default="./logs/candidate_support_audit")
    ...
    parser.add_argument(
        "--history_pose_source",
        choices=(HABITAT_GT_POSE_PROVIDER, AMB3R_VO_POSE_PROVIDER),
        default=HABITAT_GT_POSE_PROVIDER,
        help=(
            "Pose source for the frozen heatmap head. The default preserves "
            "the existing Habitat GT-c2w path; amb3r_vo_da3 explicitly uses "
            "a separate causal VO RPC server."
        ),
    )
    parser.add_argument(
        "--amb3r_vo_rpc_server",
        ...
        default="",
        help=(
            "Independent AMB3R-VO RPC host:port, required only with "
    ...
    parser.add_argument(
        ...
        type=int,
        default=600000,
        help="Timeout for reset/ingest/query calls to the independent VO server.",
    )
    parser.add_argument(
        ...
        default=95,
        help=(
            "JPEG quality for native-resolution front RGB sent to VO. "
            "No client-side spatial resize is applied."
        ),
    )
    parser.add_argument(
        "--rpc_timeout_ms",
    ...
    parser.add_argument(
        ...
        choices=("off", "on", "mismatched"),
        required=True,
        help=(
            "Fail closed unless every heatmap-control RPC response reports and "
            "exercises the requested experimental arm consistently."
        ),
    )
    parser.add_argument("--expected_heatmap_checkpoint_sha256", required=True)
    parser.add_argument("--expected_control_checkpoint_sha256", required=True)
    parser.add_argument("--expected_native_model_manifest_sha256", required=True)
    parser.add_argument(
    ...
    parser.add_argument(
        "--candidate_audit_max_gb_per_shard",
        type=float,
        default=10.0,
        help="Hard compressed-array quota per independent worker shard.",
    )
    parser.add_argument(
        "--candidate_audit_deployment_arm",
        choices=("native", "heatmap_control"),
        default="native",
        help="Fail closed unless the RPC server executes this frozen arm.",
    )
    parser.add_argument(
        "--candidate_audit_success_radius_m",
        type=float,
        default=3.0,
        help="Euclidean radius used only for local entered/left-radius diagnostics.",
    )
    parser.add_argument(
        "--oracle_system2",
        action="store_true",
    ...
    if not args.rpc_server:
        raise RuntimeError("candidate support audit requires --rpc_server")
    if args.oracle_system2:
        raise RuntimeError("candidate support audit refuses privileged oracle System2")
    if args.expected_control_mode != "on":
        raise RuntimeError("paired candidate support audit requires --expected_control_mode on")
    if not args.rpc_require_deterministic_sampling:
        raise RuntimeError("candidate support audit requires deterministic RPC sampling")
    if args.history_pose_source == AMB3R_VO_POSE_PROVIDER:
        if not str(args.amb3r_vo_rpc_server).strip():
            raise ValueError(
                "amb3r_vo_da3 pose source requires --amb3r_vo_rpc_server"
            )
    ...
    if not 0 <= int(args.candidate_audit_shard_id) < 8:
        raise ValueError("candidate_audit_shard_id must be in [0,7]")
    per_shard_gb = float(args.candidate_audit_max_gb_per_shard)
    if not math.isfinite(per_shard_gb) or not 0.0 < per_shard_gb <= 37.5:
        raise ValueError(
            "candidate_audit_max_gb_per_shard must be in (0,37.5] so eight shards stay <=300GB"
        )
    radius = float(args.candidate_audit_success_radius_m)
    if not math.isfinite(radius) or radius <= 0.0:
        raise ValueError("candidate_audit_success_radius_m must be positive")
    if float(args.auto_stop_distance) > 0.0:
        raise ValueError(
            "candidate audit requires --auto_stop_distance <= 0 to preserve native STOP semantics"
        )
    expected_runtime_options = {
        "trajectory_selection": (str(args.trajectory_selection), "mean"),
        "trajectory_x_sign": (float(args.trajectory_x_sign), 1.0),
        "trajectory_heading_alignment": (
            str(args.trajectory_heading_alignment),
            "none",
        ),
        "num_history<=8": (int(args.num_history) <= 8, True),
    }
    mismatches = {
        key: {"actual": actual, "expected": expected}
        for key, (actual, expected) in expected_runtime_options.items()
        if actual != expected
    }
    if mismatches:
        raise ValueError(f"candidate audit runtime option mismatch: {mismatches}")
    _resolve_eval_paths(args, split=str(args.dataset_split))
    run_eval(args)
