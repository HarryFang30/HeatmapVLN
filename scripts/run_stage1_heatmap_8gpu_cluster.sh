...
VAL_ROOT="${VAL_ROOT:?VAL_ROOT is required}"

RUN_NAME="${RUN_NAME:-stage1_heatmap_8gpu}"
...
CHECKPOINT_PREFERENCE="${CHECKPOINT_PREFERENCE:-best}"
...
export DATA_ROOT VAL_ROOT GPU_DEVICES OUT_DIR TB_DIR
export NUM_WORKERS PREFETCH_FACTOR PIN_MEMORY
export EPOCHS BATCH_SIZE GRAD_ACCUM_STEPS FEISHU_NOTIFY
...
cfg["data"]["val_root"] = os.environ["VAL_ROOT"]
...
cfg.setdefault("log", {}).setdefault("notify", {})["enabled"] = (
    os.environ["FEISHU_NOTIFY"].strip().lower() in {"1", "true", "yes", "on"}
)
...
CUDA_VISIBLE_DEVICES="$GPU_DEVICES" torchrun \
  ...
  --epochs "$EPOCHS" 2>&1 | tee "$TRAIN_LOG"
