...
OUTPUT_DIR="${SYSTEM2_TEMPORAL_ENSEMBLE_OUT_DIR:-${FJL_ROOT}/model/output_system2_temporal_stop_verifier_ensemble_train_rollouts_pilot}"
...
if [[ -e "$OUTPUT_DIR" ]]; then
  echo "Refusing to overwrite temporal STOP ensemble output: $OUTPUT_DIR" >&2
  exit 1
fi
...
echo "[temporal-stop-ensemble] static_prior=$STATIC_STOP_HEAD"
echo "[temporal-stop-ensemble] output=$OUTPUT_DIR gpu=$GPU_DEVICE labels=${#LABEL_FILES[@]}"
...
if summary.get("aggregation") != "unanimous" or summary.get("ensemble_size") != 5:
    raise SystemExit(f"Unexpected temporal STOP ensemble postflight: {summary}")
print(f"Temporal STOP ensemble postflight passed: {summary}")
PY

echo "[temporal-stop-ensemble] complete: $OUTPUT_DIR/latest.pth"
