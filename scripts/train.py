        ...
        if has_panoramic:
            ...
            for b in range(B):
                ...
                for v in range(4):
                    ...
                    pred_sig = pred_4[v].detach().float()
                    _logits_v = torch.logit(pred_sig.clamp(1e-6, 1 - 1e-6))
                    pred_prob = torch.softmax(_logits_v.reshape(-1), dim=0).reshape_as(pred_sig).cpu().numpy()
                    pred_vmax = max(pred_prob.max(), 1e-6)
                    axes[v, 2].imshow(pred_prob, cmap='inferno', vmin=0, vmax=pred_vmax)
                    pr = pred_prob.max() / (1.0 / (pred_prob.shape[0] * pred_prob.shape[1]))
                    axes[v, 2].set_title(f"Pred ({pr:.0f}× unif)")
                    axes[v, 2].axis('off')
