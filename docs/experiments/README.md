...
在专家轨迹上的另一种体现（侧后三视角 IoU 合计 < 0.006）。所以：

- H1 里"注入是否改变**方向选择**"这一问已被 EXP-12 大致答掉了——**没有可改的方向**，
  桥关臂大概率也只会得到同样的常数，差值天然趋近 0；
- 仍然**没有**被答掉的是"注入是否改变**前视热力图内部的空间分布**"。Soft-IoU 与 top-k 召回
  测的正是这个，两者对桥开/关的敏感度仍然是未知的。
