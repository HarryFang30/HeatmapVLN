...
| [EXP-10](#exp-10-未来认知头与桥的关联是否可观测) | 注入的历史记忆是否改变未来预测（关联的可观测性）；Z 的第 i 个向量是否真的绑定第 i 段 | ⚠️ | **H1 支持但很弱**：三项判据只有 top-k 召回过线（+1.13pt，线是 1pt），Soft-IoU 方向还是反的。机制是注入把少量质量从前视挪向侧视（侧视相对 +56%，基数近零）——**关联可观测，但小到不足以改变任何决策**（argmax 仍恒为前，EXP-12）。H2 仍未测 |
...
**H2 仍未测.** 探针工具仍未实现（单卡，可在开发机跑）。
⇒ **"未来头把 Z 的第 i 个向量解码成第 i 段路点"目前没有任何证据**，既不能声称也未被否掉。
