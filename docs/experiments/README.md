...
| [EXP-16](#exp-16-进度是否已经可读到达读出能否在集级净赚) | 慢系统隐状态是否已含"进度 / 到达"信息、超出时间记账量；用现成特征读出"到达"并覆盖成 STOP，在集级能否净赚 | 🔬 | 判据已预注册（2026-09-06），探针运行中；3a 门决定零训练覆盖金丝雀跑不跑 |
...
| EXP-16 进度/到达读出 + 3a 净收益曲线（判据来源，**运行中**） | `model/exp16_progress_probe/readout_progress.json` + `probe.log` |
