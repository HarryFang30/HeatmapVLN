...
| [EXP-15](#exp-15-几何优势扛不扛得住部署期的位姿误差) | 13-A 的几何优势用的是 Habitat 真值位姿，换成部署期的 VO 量级噪声还在不在 | ⏳ | 已预注册：跑在 13-A 已缓存的特征上，**纯 CPU**，**应排在 EXP-14 的 7 卡时之前**——它可能直接否掉整条几何线 |
...
| 环 | 内容 | 谁来测 | 风险 |
|---|---|---|---|
| A **可读 → 可学** | 线性探针说信息在缓存特征里；不等于 LoRA 微调能让 System2 用上它 | EXP-14（=顶替 13-B） | 中。同一份特征，换成 token 进提示，模型得自己学会读 |
| B **决策级 → 闭环** | 首 token 从"前"变成"→"，不等于 SR 涨；还要经过执行、时序、整集其余步骤 | 14-C | 中高。EXP-01→05 的教训就是决策级与闭环脱节 |
| C **真值位姿 → VO** | 13-A 的几何量来自 DAgger tar 里的 **Habitat 真值**；部署期是 AMB3R VO | **无人测** | **最高**。几何臂的全部优势就是位姿；EXP-04 测到位姿域偏移把 pck8 从 0.88 压到 0.66 |

**环 C 是当前最大的未知，也是最便宜的一个。** 13-A 的特征已经全部缓存
（`features_merged.npz`），给 `history_rel_poses` 加不同幅度的噪声再重拟合读出头，
**纯 CPU、分钟级**，就能画出"几何优势随位姿噪声怎么衰减"。
它不等于真 VO（真误差是相关的、漂移的，不是 iid），所以**只能否证不能证实**：
优势在小噪声下就塌，那 13-C 之前不必再投；扛得住也只是"没被否掉"，
仍要按 13-C 的要求给 DAgger 集补 VO 缓存复验。
**这个探针应该排在 EXP-14 的 7 个卡时之前**——它可能直接告诉你别烧那 7 小时。
...
| EXP-15 位姿噪声扫描（**待跑**，纯 CPU，判据来源） | `model/exp15_pose_noise/readout_noise_sweep.json` |
