...
| [EXP-10](#exp-10-未来认知头与桥的关联是否可观测) | 注入的历史记忆是否改变未来预测（关联的可观测性）；Z 的第 i 个向量是否真的绑定第 i 段 | ⚠️ | **H1 支持但很弱**（三项只有 top-k 过线 +1.13pt，Soft-IoU 方向还反）：注入把少量质量从前视挪向侧视——**关联可观测，但小到不足以改变任何决策**。**H2 没测出来**（主臂降 5.23%，否定线 5%，仅高 0.23pt；四个排列臂里三个落在否定带）。附带测到 **Z 换成别的样本也只掉 7.88%** |
...
⇒ 论文里**两句都不能写**：既不能写"第 i 个向量负责第 i 段"（离支持线差得远），
也不能援引判据写成"共同解码"（那是否定档措辞，本次未触发否定）。
最保守是不提这层结构。补一次留逐批统计量的重跑（36 分钟单卡）即可给出配对 bootstrap 的 CI，
把 5.23% 钉死在否定带内还是带外。
