...
readonly LAUNCHER="${STAGE_ROOT}/scripts/run_past_plan_action_4gpu_mxc500.sh"
readonly WEBSITE="${STAGE_ROOT}/docs/experiments/legacy/ppa-stage12-submission-4gpu.md"
...
common_env=(
  ...
  PPA_GPU_DEVICES=0,1,2,3
)

# Keep the copy/paste website contracts tied to the formal flat R2R scene root,
# full two-split cache coverage, the four-GPU wrapper, and the current Past
# initializer. These assertions are intentionally exact enough to catch a
...
grep -F 'bash scripts/run_amb3r_pose_training_cache_4gpu_mxc500.sh' \
  "$WEBSITE" >/dev/null
grep -F \
