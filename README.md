...
| 数据项 | 范围 | 说明 |
|--------|------|------|
| **历史帧** | `[subseq_start, current_t)` | 不含当前帧 |
| **热力图** | 历史帧 → 当前帧视角 | 历史位置投影 |
| **轨迹** | `[current_t, subseq_end)` | 到子序列结束 |
| **Progress** | `(t - start + 1) / length` | 基于子序列长度 |
| **指令** | FGR2R 子指令 | 对应子序列范围 |

**关键对齐**：轨迹计算到 `subseq_end` 而非 clip 结束，确保 `progress=1` 时轨迹为零（语义一致）
...
| **区域感知损失** | 对稀疏区域（前方、上下）增加权重，解决热力图分布不均问题 |
| **平方根归一化** | 缓解稀疏热力图的分布不均匀问题 |
| **360° Circular Padding** | 支持全景图的水平边界连续性 |

**区域感知损失 (Regional Focal Loss)**：

针对热力图分布不均匀问题（89% 集中在垂直中间，80% 集中在左右边缘/后方），对前方区域给予更高权重：
...
- **中心区域（前方）**：权重 ×2.5，提升对导航目标方向的预测（20% → 39%）
- **上下区域**：权重 ×1.0（不加权），避免在无效数据上过拟合
- **区域损失权重**：占总损失 20%
...
model:
  heatmap_head:
    ...
    # 区域感知损失 (Regional Focal Loss)
    regional_loss_enabled: true   # 启用区域感知损失
    regional_center_alpha: 2.5    # 中心区域（前方）权重倍数 (20% → 39%)
    regional_vertical_alpha: 1.0  # 上下区域权重倍数 (不加权)
    regional_loss_weight: 0.2     # 区域损失占总损失的权重
