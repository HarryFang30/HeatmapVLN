...
针对热力图分布不均匀问题（89% 集中在垂直中间，80% 集中在左右边缘/后方），对稀疏区域给予更高权重：
...
- **中心区域（前方）**：权重 ×2.0，提升对导航目标方向的预测
- **上下区域**：权重 ×1.5，关注垂直方向稀疏点
- **区域损失权重**：占总损失 20%
...
model:
  heatmap_head:
    ...
    regional_center_alpha: 2.0    # 中心区域（前方）权重倍数
    regional_vertical_alpha: 1.5  # 上下区域权重倍数
