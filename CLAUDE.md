...
**所有实验记在 `docs/EXPERIMENTS.md`，不要只留在聊天里。** 那份文档记的是
*为什么跑、判据是什么、结论能支撑到哪一步*，结果本身在各自的 `docs/*_report.md` 里。

跑新实验之前：

- 先读台账 §4（基线数字、金参照、数据集路径）和 §5（反复踩过的坑），别重新发现一遍。
- **开跑前先写"问题 / 假设 / 判据 / 设置"四段并 commit。判据必须在看到任何结果之前
  写死**——什么数值算支持、什么算否定、什么算没测出来。事后补的判据没有约束力。
- 跑完补"结果 / 结论 / 边界"。"边界"是强制的：这个结论**不能**推广到哪里。
